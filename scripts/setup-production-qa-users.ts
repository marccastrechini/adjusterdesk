...
async function main() {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      await tx.passwordResetToken.deleteMany({ where: { userId: { in: [qaUser.id, qaAdmin.id] } } });
      await tx.userInvitationToken.deleteMany({ where: { userId: { in: [qaUser.id, qaAdmin.id] } } });

      const qaEnvPath = writeLocalQaCredentials({
        repoRoot,
        userEmail,
        userPassword,
        adminEmail,
